...
#### Create a custom widget field

To create a new custom widget you must have a custom node js module on your project (as "mycustommodule"). You must not override directly the files in vendor folder.

You must create your custom widget file : 

* ``custom wigdet file`` - mycustommodule/forms/custom/widgets.js
...
* ``form generation file`` - mycustommodule/forms/form.js

Then, during the generation of the field, call the new widget :
```javascript
...
```
The current field is now a media field an depends on exports.media method.
